      ...
      <Container className="wide">
        <Row>
          {categoryData.map((category, index) => {
            ...
            const productOnline = productData.filter(
              (product) => product.supplierCategoryId === category._id
            )
              ? productData.filter(
                  (product) => product.supplierCategoryId === category._id
                ).length
              ...
              return (
                <Col lg={3} md={6}>
                  <div className="single-category single-category--one space-mb--5">
                    <Card>
                      <Card.Header style={{ backgroundColor: "#FBFBFF" }}>
                        ...
                        <span style={{ color: "#31de79" }}>
                          {productData.filter(
                            (product) =>
                              product.supplierCategoryId === category._id
                          )
                            ? productData.filter(
                                (product) =>
                                  product.supplierCategoryId === category._id
                      ...
                      <Card.Body>
                        ...
                        <Accordion
                          expanded={
                            ...
                            category._id === categoryData[indexes[0]]._id
                          }
                          className={classes.root}
                          onChange={handleChangePanel(category._id)}
                        >
                          <AccordionSummary
